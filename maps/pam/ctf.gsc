...
flag()
{
	...
	for(;;)
	{
		...
		//PAM
		if (level.mapended || level.hithalftime)
		{
			wait level.fps_multiplier * 1;
			continue;
		}
			...
			if(other.pers["team"] == self.team) // Touched by team
			{
				if(self.atbase)
				{
					if(isdefined(other.flag)) // Captured flag
					{
						...
						//PAM
						if (level.ctf_capture_points)
							other.score = other.score + level.ctf_capture_points;
				...
				else // Returned flag
				{
					...
					//other.score += 2;
					if (level.ctf_return_points)
							other.score = other.score + level.ctf_return_points;
					level notify("update_allhud_score");
				}
			}
...
autoReturn()
{
	...
	level.flag_auto_return = getcvarInt("scr_flag_autoreturn_time");
...
{ 
   ...
   { 
      player = players[i]; 
      if(isdefined(player.WaitingToSpawn)) 
      { 
...
{
	self endon("end_autoreturn");
	time = getcvarInt("scr_flag_autoreturn_time");
	if (time == 0)
		return;
