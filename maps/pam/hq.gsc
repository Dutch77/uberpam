...
Callback_StartGameType()
{
	...
	if(getcvar("scr_hq_RadioSpawnDelay") == "")
		setcvar("scr_hq_RadioSpawnDelay", "15");
	else if (getcvarint("scr_hq_RadioSpawnDelay") < 5)
		setcvar("scr_hq_RadioSpawnDelay", "5");
	level.RadioSpawnDelay = getcvarint("scr_hq_RadioSpawnDelay");
...
startGame()
{
	...
	if (game["mode"] == "match" && (!isDefined(game["matchstarted"]) || !game["matchstarted"]) )
	{
...
hq_setup()
{
	...
	if (game["matchstarted"] && !game["Do_Ready_up"])
	...
	if(NeutralRadios <= 0)
	{
		...
		if(level.radio.size >= 4)
		{
			...
			if (level.orig_hq_style)
			{
				...
				thread maps\pam\objpoints::addObjpoint(fakeposition.origin, "1", game["radio_prespawn_objpoint"][randAorB]);
			}
		}
		...
		if (level.orig_hq_style)
		{
			...
			thread maps\pam\objpoints::addObjpoint(level.radio[level.nextradio].origin, "0", game["radio_prespawn_objpoint"][otherAorB]);
		}
		...
		wait level.fps_multiplier * level.RadioSpawnDelay;
		...
		thread maps\pam\objpoints::removeObjpoints();
		thread maps\pam\objpoints::addObjpoint(level.radio[level.nextradio].origin, "0", "objpoint_radio");
	...
	if(team == "none")
	{
		...
		if(isdefined(NeutralizingTeam))
		{
			if((NeutralizingTeam == "allies") || (NeutralizingTeam == "axis"))
			{
				...
				{
					HQ_Scoring(NeutralizingTeam, level.NeutralizingPoints);
	...
	if(level.RadioMaxHoldSeconds > 0)
	{
		...
		while (1)
		{
			...
			if (current_time >= next_point_time)
			{
				...
				{
					HQ_Scoring(level.DefendingRadioTeam, 1);
...
HQ_Scoring(Team, Points)
{
	...
	{
		if (game["halftimeflag"])
			game["half_2_allies_score"] += Points;
		else
			game["half_1_allies_score"] += Points;
	}
	else if (Team == "axis")
	{
		if (game["halftimeflag"])
			game["half_2_axis_score"] += Points;
		else
			game["half_1_axis_score"] += Points;
	}
