...
{ 
	...
	setcvar("scr_flag_autoreturn_time", 60);
	setcvar("scr_ctf_respawn_time", 8);
	...
	//OT Rules 
	if (getcvarint("g_ot_active") > 0) //1st OT 
	{ 
		setcvar("scr_ctf_timelimit", 5); 
	...
	//CTF
	setcvar("scr_ctf_show_compassflag", 0);
	setcvar("scr_ctf_flag_graceperiod", 30);
	setcvar("scr_ctf_positiontime", 10);
	setcvar("scr_ctf_show_flagstatus_hud", 0);
	...
	// CTF Scoring Options
	setcvar("scr_ctf_capture_points", 10); //10 stock
	setcvar("scr_ctf_return_points", 2); //2 stock

	// Respawn on Capture
	setcvar("scr_ctf_respawn_on_capture", 0); // if on (1) everyone will respawn when a flag is captured
	...
	game["leaguestring"] = &"CyberGamer CTF 2v2 Ladder Match Mode"; 
	game["mode"] = "match"; 
}